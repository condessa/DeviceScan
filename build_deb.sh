...
DESCRIPTION="Fast network scanner with vendor identification"
...
if ! ${PYTHON} -c "import PIL" 2>/dev/null; then
    echo "ERRO: Pillow não encontrado."
...
Comment=Fast network scanner with vendor identification — by HCsoftware
Exec=/opt/devicescan/device_scan
Icon=hcsoftware
Type=Application
Categories=Network;System;Utility;
Keywords=network;scan;ip;mac;vendor;wifi;lan;
StartupNotify=true
...
devicescan (1.0.0) stable; urgency=low

  * Initial release
  * Network scanner with IEEE OUI vendor identification
  * Multi-language: PT, EN, ES, FR
  * Random MAC detection
  * New device notifications

 -- HCsoftware <hcsoftware@example.com>  Mon, 24 Mar 2026 00:00:00 +0000
CHANGELOG
...
# Atualizar menu de aplicações
if command -v update-desktop-database &>/dev/null; then
    update-desktop-database /usr/share/applications 2>/dev/null || true
fi
echo "Device Scan instalado com sucesso!"
echo "Execute: device-scan  (ou pelo menu de aplicações)"
POSTINST
...
if [ -f "${DEB_FILE}" ]; then
    ...
    printf "║  Tamanho:  %-43s ║\n" "${SIZE}"
    echo "╠══════════════════════════════════════════════════════╣"
    echo "║  Para instalar no PC de destino:                    ║"
