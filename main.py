...
class I18n:
    ...
    _BUILTIN = {
        "pt": {
            ...
            "tip_scan":                "Iniciar scan da rede no intervalo de IPs definido",
            "tip_stop":                "Interromper o scan em curso",
            "tip_export":                "Exportar resultados para CSV, JSON ou TXT",
            "tip_oui":                "Atualizar a base de dados de fabricantes (IEEE OUI)Requer ligacao a internet",
            "tip_mqtt":                "Configurar MQTT. \nToggle Duplo clique no Status do dispositivo envia TOGGLE",
            "tip_theme_dark":                "Mudar para tema claro",
            "tip_theme_light":                "Mudar para tema escuro",
            "tip_lang":                "Selecionar idioma da interface",
        ...
        "en": {
            ...
            "tip_scan":                "Start network scan in the defined IP range",
            "tip_stop":                "Stop the current scan",
            "tip_export":                "Export results to CSV, JSON or TXT",
            "tip_oui":                "Update the manufacturer database (IEEE OUI)Requires internet connection",
            "tip_mqtt":                "Configure MQTT. \nToggle Double-click on a Status device to send TOGGLE",
            "tip_theme_dark":                "Switch to light theme",
            "tip_theme_light":                "Switch to dark theme",
        ...
        "es": {
            ...
            "tip_scan":                "Iniciar scan de la red en el intervalo de IPs definido",
            "tip_stop":                "Detener el scan en curso",
            "tip_export":                "Exportar resultados a CSV, JSON o TXT",
            "tip_oui":                "Actualizar la base de datos de fabricantes (IEEE OUI)Requiere conexion a internet",
            "tip_mqtt":                "Configurar MQTT. \nToggle Doble clic en el Status del dispositivo envia TOGGLE",
            "tip_theme_dark":                "Cambiar a tema claro",
            "tip_theme_light":                "Cambiar a tema oscuro",
            "tip_lang":                "Seleccionar idioma de la interfaz",
            "notif_new":         "¡Nuevo dispositivo detectado!",
        },
        "fr": {
            "app_title":         "Scanner Réseau",
            "lbl_network":       "Réseau:",
            "lbl_ip":            "IP:",
            "lbl_to":            "à",
            "lbl_language":      "Langue:",
            ...
            "tip_scan":                "Demarrer le scan reseau dans la plage d'IPs definie",
            "tip_stop":                "Arreter le scan en cours",
            "tip_export":                "Exporter les resultats en CSV, JSON ou TXT",
            "tip_oui":                "Mettre a jour la base de fabricants (IEEE OUI)Necessite une connexion internet",
            "tip_mqtt":                "Configurer MQTT Toggle. \nDouble-clic sur un Status appareil envoie TOGGLE",
            "tip_theme_dark":                "Passer au theme clair",
            "tip_theme_light":                "Passer au theme sombre",
            "tip_lang":                "Selectionner la langue de l'interface",
...
# ------------------------------------------------------------------
# MQTT TOGGLE + AUTO-DESCOBERTA DE TOPICS
# ------------------------------------------------------------------

MQTT_CFG_FILE = os.path.join(get_data_dir(), "mqtt_toggle_config.json")


def _load_mqtt_cfg() -> dict:
    defaults = {"broker": "192.168.1.1", "port": 1883,
                "username": "", "password": "", "enabled": False}
    if os.path.exists(MQTT_CFG_FILE):
        try:
            with open(MQTT_CFG_FILE, "r", encoding="utf-8") as f:
                loaded = json.load(f)
...
def _mac_to_ip_map() -> dict:
    """Le /proc/net/arp e devolve {mac_upper: ip}."""
    result = {}
    try:
        with open("/proc/net/arp", "r") as f:
            for line in f.readlines()[1:]:
...
class MQTTTopicDiscovery:
    """
    Listener MQTT em background.
    Subscreve tele/+/LWT e tele/+/STATE.
    Quando um dispositivo Tasmota aparece online, tenta associar
    o topic ao IP via MAC address (STATE contem MAC) ou ARP.
    Guarda mqtt_topic em known_devices automaticamente.
    """
    def __init__(self, known_devices: dict, save_cb, notify_cb=None):
        self.known_devices = known_devices
        self.save_cb       = save_cb
        self.notify_cb     = notify_cb  # funcao(ip, topic) para UI
        self._client       = None
        self._running      = False
        self._mac_topic    = {}  # {mac_upper: topic}

    def start(self, cfg: dict):
        if not _MQTT_AVAILABLE or not cfg.get("enabled", False):
            return
        self._running = True
        ...
        try:
            try:
                c = _mqtt_lib.Client(
                    ...
                    client_id="devicescan_discovery")
            except AttributeError:
                c = _mqtt_lib.Client(client_id="devicescan_discovery")
            if cfg.get("username"):
            ...
            self._client = c
            c.connect(cfg["broker"], int(cfg["port"]), keepalive=60)
            c.loop_forever()
        except Exception as e:
            print(f"[MQTT Discovery] Erro: {e}")

    def _on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            client.subscribe("tele/+/LWT")
            client.subscribe("tele/+/STATE")
            client.subscribe("stat/+/STATUS5")  # contem IP e MAC
            print("[MQTT Discovery] Ligado, a ouvir topics Tasmota...")
    ...
    def _on_message(self, client, userdata, msg):
        ...
        if len(parts) < 3:
            return
        dev_topic = parts[1]  # ex: 'fluorescente'

        # tele/{topic}/STATE contem {"Wifi":{"IPAddress":"...","MAC":"..."}}
        if parts[2] == "STATE":
            self._handle_state(dev_topic, payload)
            return

        # stat/{topic}/STATUS5 contem {"StatusNET":{"IPAddress":"...","Mac":"..."}}
        if parts[0] == "stat" and parts[2] == "STATUS5":
            self._handle_status5(dev_topic, payload)
            return

        # tele/{topic}/LWT -> pedir STATUS5 para obter IP/MAC
        if parts[2] == "LWT" and payload.lower() == "online":
            self._mac_topic[dev_topic] = dev_topic  # guardar provisoriamente
            if self._client:
                self._client.publish(f"cmnd/{dev_topic}/STATUS", "5")
            # Tentar associar por ARP imediatamente
            self._try_arp_match(dev_topic)

    def _handle_state(self, dev_topic: str, payload: str):
        try:
            d = json.loads(payload)
            wifi = d.get("Wifi", {})
            ip   = wifi.get("IPAddress", "").strip()
            mac  = wifi.get("MAC", "").upper().replace("-", ":")
            if ip:
                self._associate(ip, mac, dev_topic)
            elif mac:
                mac_map = _mac_to_ip_map()
                if mac in mac_map:
                    self._associate(mac_map[mac], mac, dev_topic)
        except Exception:
            pass

    def _handle_status5(self, dev_topic: str, payload: str):
        try:
            d   = json.loads(payload)
            net = d.get("StatusNET", {})
            ip  = net.get("IPAddress", "").strip()
            mac = net.get("Mac", "").upper().replace("-", ":")
            if ip:
                self._associate(ip, mac, dev_topic)
            elif mac:
                mac_map = _mac_to_ip_map()
    ...
    def _try_arp_match(self, dev_topic: str):
        """Tenta associar dev_topic a um IP via MAC no ARP + known_devices."""
        mac_map = _mac_to_ip_map()  # {mac: ip}
        # Procurar em known_devices um IP cujo MAC esteja no ARP
        for ip, kd in self.known_devices.items():
            if kd.get("mqtt_topic"):
                continue  # ja tem topic
            mac = kd.get("mac", "").upper()
            if mac and mac in mac_map:
                # Este IP esta online agora - mas nao sabemos o topic ainda
                pass

    def _associate(self, ip: str, mac: str, dev_topic: str):
        """Associa ip <-> topic e guarda em known_devices."""
        changed = False
        if ip in self.known_devices:
            kd = self.known_devices[ip]
            if kd.get("mqtt_topic") != dev_topic:
                kd["mqtt_topic"] = dev_topic
                if mac and not kd.get("mac"):
                    kd["mac"] = mac
                changed = True
                print(f"[MQTT Discovery] {ip} -> topic '{dev_topic}'")
                if self.notify_cb:
                    self.notify_cb(ip, dev_topic)
        else:
            # Dispositivo novo - procurar por MAC no ARP
            if mac:
                mac_map = _mac_to_ip_map()
                arp_ip  = mac_map.get(mac)
                if arp_ip and arp_ip in self.known_devices:
                    kd = self.known_devices[arp_ip]
                    if kd.get("mqtt_topic") != dev_topic:
                        kd["mqtt_topic"] = dev_topic
                        changed = True
                        print(f"[MQTT Discovery] {arp_ip} (via MAC) -> topic '{dev_topic}'")
                        if self.notify_cb:
                            self.notify_cb(arp_ip, dev_topic)
        if changed:
            self.save_cb()

    def request_all_status(self):
        """Pede STATUS5 a todos os dispositivos conhecidos com topic."""
        if not self._client:
            return
        seen = set()
        for kd in self.known_devices.values():
            ...
            if t and t not in seen:
                self._client.publish(f"cmnd/{t}/STATUS", "5")
                seen.add(t)


class MQTTToggleManager:
    def __init__(self):
        self.cfg = _load_mqtt_cfg()

    def send_toggle(self, hostname: str, on_result=None):
        self.cfg = _load_mqtt_cfg()
        if not _MQTT_AVAILABLE:
            if on_result: on_result(False, "paho-mqtt nao instalado")
            return
        if not self.cfg.get("enabled", False):
            if on_result: on_result(False, "MQTT desativado")
            return
        if not hostname:
            if on_result: on_result(False, "Sem topic MQTT")
            return

        def _worker():
            topic = f"cmnd/{hostname}/POWER"
            try:
                try:
                    c = _mqtt_lib.Client(
                        callback_api_version=_mqtt_lib.CallbackAPIVersion.VERSION1,
                        client_id="devicescan_toggle")
                except AttributeError:
                    c = _mqtt_lib.Client(client_id="devicescan_toggle")
                if self.cfg.get("username"):
                    c.username_pw_set(self.cfg["username"],
                                      self.cfg.get("password", ""))
                c.connect(self.cfg["broker"], int(self.cfg["port"]), keepalive=10)
                c.loop_start()
                import time as _t; _t.sleep(1)
                r = c.publish(topic, "TOGGLE", qos=1)
                r.wait_for_publish(timeout=5)
                c.loop_stop(); c.disconnect()
                if on_result: on_result(True, f"TOGGLE -> {topic}")
            except Exception as e:
                if on_result: on_result(False, f"MQTT erro: {e}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.mqtt_toggle    = MQTTToggleManager()
        self.mqtt_discovery = MQTTTopicDiscovery(
            self.known_devices,
            self._save_known_devices,
            notify_cb=self._on_topic_discovered
        )
        self.notif        = NotificationManager(QApplication.instance(), self)
        self.status_timer = QTimer()
        # Iniciar discovery apos construcao
        QTimer.singleShot(2000, self._start_mqtt_discovery)
    ...
    def _setup_ui(self):
        ...
        self.lang_combo.setToolTip(TR.t("tip_lang"))
        ...
        self.theme_btn.setToolTip(TR.t("tip_theme_dark"))
        ...
        self.scan_btn.setToolTip(TR.t("tip_scan"))
        ...
        self.oui_btn.setToolTip(TR.t("tip_oui"))
        ctrl_layout.addWidget(self.oui_btn)

        self.mqtt_btn = QPushButton("MQTT")
        self.mqtt_btn.setMaximumWidth(55)
        self.mqtt_btn.setFixedHeight(24)
        self.mqtt_btn.setToolTip("Configurar MQTT Toggle\nDuplo clique no Status do dispositivo envia TOGGLE")
        self.mqtt_btn.clicked.connect(self._open_mqtt_config)
        ctrl_layout.addWidget(self.mqtt_btn)
    ...
    def _apply_styles(self):
        if self.dark_mode:
            ...
            self.theme_btn.setToolTip(TR.t("tip_theme_light"))
    ...
    def _refresh_ui_texts(self):
        ...
        self.oui_btn.setToolTip(TR.t("tip_oui"))
        if hasattr(self, 'mqtt_btn'):
            self.mqtt_btn.setToolTip(TR.t("tip_mqtt"))
        self.lang_combo.setToolTip(TR.t("tip_lang"))
        self.table.setHorizontalHeaderLabels([
            TR.t("col_status"), TR.t("col_device"),
            TR.t("col_mac"),    TR.t("col_hostname"), TR.t("col_vendor")
    ...
    def _start_mqtt_discovery(self):
        cfg = _load_mqtt_cfg()
        self.mqtt_discovery.start(cfg)
        if cfg.get("enabled"):
            self.status_label.setText("MQTT Discovery ativo")
            # Apos 3s pedir STATUS5 a todos os dispositivos conhecidos
            QTimer.singleShot(3000, self.mqtt_discovery.request_all_status)

    def _on_topic_discovered(self, ip: str, topic: str):
        """Chamado pelo MQTTTopicDiscovery quando associa ip <-> topic."""
        from PyQt5.QtCore import QTimer
        QTimer.singleShot(0, lambda: self.status_label.setText(
            f"Topic descoberto: {ip} -> '{topic}'"))

    def _open_mqtt_config(self):
        cfg = _load_mqtt_cfg()
        dlg = QDialog(self)
        dlg.setWindowTitle("Configuracao MQTT")
        dlg.setModal(True)
        dlg.setMinimumWidth(320)
        dlg.setStyleSheet("background-color: #2b2b2b; color: #f0f0f0;")
        from PyQt5.QtWidgets import QCheckBox
        layout = QVBoxLayout(dlg)
        layout.setContentsMargins(16, 16, 16, 12)
        chk = QCheckBox("Ativar MQTT Toggle + Discovery")
        chk.setChecked(cfg.get("enabled", False))
        chk.setStyleSheet("color: #f0f0f0;")
        layout.addWidget(chk)
        form = QFormLayout()
        e_broker = QLineEdit(cfg.get("broker", "192.168.1.1"))
        form.addRow("Broker IP:", e_broker)
        e_port = QLineEdit(str(cfg.get("port", 1883)))
        form.addRow("Porta:", e_port)
        e_user = QLineEdit(cfg.get("username", ""))
        form.addRow("Utilizador:", e_user)
        e_pass = QLineEdit(cfg.get("password", ""))
        e_pass.setEchoMode(QLineEdit.Password)
        form.addRow("Password:", e_pass)
        layout.addLayout(form)
        note = QLabel("O Discovery associa automaticamente os topics\n"
                      "Tasmota aos IPs via mensagens MQTT.")
        ...
        if dlg.exec_() == QDialog.Accepted:
            try:
                new_cfg = {"enabled": chk.isChecked(),
                           "broker":  e_broker.text().strip(),
                           "port":    int(e_port.text().strip()),
                           "username":e_user.text().strip(),
                           "password":e_pass.text().strip()}
                with open(MQTT_CFG_FILE, "w", encoding="utf-8") as f:
                    json.dump(new_cfg, f, indent=2)
                self.mqtt_toggle.cfg = new_cfg
                self.mqtt_discovery.stop()
                self.mqtt_discovery = MQTTTopicDiscovery(
                    self.known_devices, self._save_known_devices,
                    notify_cb=self._on_topic_discovered)
                QTimer.singleShot(500, self._start_mqtt_discovery)
                status = "MQTT ativo: " + new_cfg["broker"] if new_cfg["enabled"] else "MQTT desativado."
                self.status_label.setText(status)
                if new_cfg["enabled"]:
                    self.mqtt_btn.setStyleSheet(
                        "background-color: #27ae60; color: white;"
                        "font-weight: bold; border-radius: 3px;")
                else:
                    self.mqtt_btn.setStyleSheet("")
            except ValueError:
                self.status_label.setText("Porta invalida.")

    def _on_cell_double_clicked(self, row: int, col: int):
        ip_item = self.table.item(row, 1)
        if not ip_item:
            return
        ip = self._ip_from_display(ip_item.text())
        kd = self.known_devices.get(ip, {})
        # Prioridade: mqtt_topic > hostname > custom_name em minusculas
        topic = kd.get("mqtt_topic", "").strip()
        if not topic:
            topic = kd.get("hostname", "").strip()
        if not topic:
            custom = kd.get("custom_name", "").strip()
            if custom:
                topic = custom.lower()
        if not topic:
            self.status_label.setText(f"Sem topic para {ip} -- TOGGLE nao enviado.")
            return

        def _result(success: bool, msg: str):
            from PyQt5.QtCore import QTimer
            QTimer.singleShot(0, lambda: self.status_label.setText(msg))

        self.mqtt_toggle.send_toggle(topic, on_result=_result)
        self.status_label.setText(f"A enviar TOGGLE -> cmnd/{topic}/POWER ...")
